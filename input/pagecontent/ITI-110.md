...
This method is invoked when the Resource Notification Subscriber needs to submit a new subscription to the Resource Notification Broker or needs to update an already submitted subscription. The Resource Notification Subscriber has already discovered the SubscriptionTopic resource supported by the Resource Notification Broker. Note that the SubscriptionTopic knowledge could be accomplished by the Resource Subscription Search [[ITI-114]](ITI-114.html) (see 1:54.2.2 SubscriptionTopic Search Option) or because the SubscriptionTopic resources have been already shered with different methodologies among the community.
...
2. Multi-patient Subscription resource:
    ...
    2.1. Multi-patient Folder Subscription resource:
    ...
    - in the `subscription.criteria` element shall not be included a patient identifier (Folder.subject);
    ...
    2.2. Multi-patient Document resource:
    - shall contain in the `subscription.criteria` element the reference to an existing SubscriptionTopic  resource with `SubscriptionTopic.resourceTrigger.resource = DocumentReference`;  
