...
Based on the outcome of the conditions above, the Resource Notification Broker SHALL accept or reject the subscription resource received. If one or more of the conditions are false the Resource Notification Broker SHALL reject the Subscription resource and respond with an error message, according with which is define in [Subscription Error Response Message [ITI-110]](#231109-subscription-error-response-message). 
Otherwise, the Resource Notification Broker SHALL accept the Subscription resource, create the subscription, respond with a success message, according with which is define in [Create Subscription Response Message [ITI-110]](#231106-create-subscription-response-message) and proceed to performing the handshake process.
