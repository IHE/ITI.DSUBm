...
###### 3.111.5.1.1 Updates to document sharing resources option

When the Resource Notification Publish er support the [Updates to document sharing resources option](#Updating-events), in addition to the privious events, events that could trigger this message also are:
...
The FHIR Bundle shall have the following constrains:
 ...
 - [Publishing Bundle](StructureDefinition-IHE.DSUBm.Minimal.FolderR4.html)<!--link-->:
    ...
    - may convey one or more any other Resource that is referenced in the previous
...
The Resource Notification Publisher shall ensure to convey these resources mantaining all the information of the resources being part of the events; also the contained resourced shall be mantained using the FHIR contained method (see [http://hl7.org/fhir/R4/references.html#contained](http://hl7.org/fhir/R4/references.html#contained)).

The referenced resources generally are hosted on a commonly accessible server. If they are not be accessible by the Resource Notification Broker for the proper evaluation, they shall be included in the Bundle. Othorwise they shall not be included.
