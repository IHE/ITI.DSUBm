...
<!-- handshake-->
#### 2:3.112.5 Handshake Notification Message
...
The Resource Notification Recipient processes the message according to application-defined rules and produces a [Handshake Notification Response](#231129-handshake-notification-response).

If the Resource Notification Broker has not been able to send the [Handshake Notification Message](#231125-handshake-notification-message) because of connection problem, it shall not activate the Subscription and set the `Subscription.status=error`.

If the Resource Notification Broker has been able to send the Handshake Notification Message but it has not received any [Handshake Notification Response](#231129-handshake-notification-response) from the Resource Notification Recipient, it shall not activate the Subscription and set the `Subscription.status=error`.


<!-- heartbeat-->
#### 2:3.112.6 Heartbeat Notification Message
...
The Resource Notification Recipient processes the message according to application-defined rules and produces a [Heartbeat Notification Response](#2311210-hearthbeat-notification-response).

If the Resource Notification Broker has not been able to send the [Heartbeat Notification Message](#231126-heartbeat-notification-message) because of connection problem, it should deactivate the Subscription, setting the `Subscription.status=error`. 

If the Resource Notification Broker has been able to send the Handshake Notification Message but it has not received any [Hearthbeat Notification Message](#231126-heartbeat-notification-message) from the Resource Notification Recipient, it should deactivate the Subscription, setting the `Subscription.status=error`.

The recommended way for the Resource Notification Broker to manage these two type of error is to continue to send the Heartbeat Notification Event for a reasonable number of times, and evaluating if these type of errors continue before deactivating the Subscription, setting the `Subscription.status=error`. This should be done based on the capability of the connection infrastructure.
...
<!-- event-->
#### 2:3.112.7 Event Notification Message
...
The Resource Notification Recipient processes the message according to application-defined rules and produces an [Event Notification Response](#2311211-event-notification-response).

If the Resource Notification Broker has not been able to send the [Event Notification Message](#231127-event-notification-message) because of connection problem, it should deactivate the Subscription, setting the `Subscription.status=error`.

If the Resource Notification Broker has been able to send the Handshake Notification Message but it has not received any [Event Notification Message](#231127-event-notification-message) from the Resource Notification Recipient, it should deactivate the Subscription, setting the `Subscription.status=error`.

The recommended way for the Resource Notification Broker to manage these two type of error is to continue to send the Event Notification Event, if events occur, for a reasonable number of times, and evaluating if these type of errors continue before deactivating the Subscription, setting the `Subscription.status=error`. This should be done based on the capability of the connection infrastructure.
...
<!-- deactivation-->
#### 2:3.112.8 Subscription Deactivation Notification Message
...
The Resource Notification Recipient processes the message according to application-defined rules and produces an [Subscription Deactivation Response](#2311212-subscription-deactivation-notification-response).
...
#### 2:3.112.9 Handshake Notification Response

The Resource Notification Recipient sends a Handshake Notification Response to respond to an [Handshake Notification Message](#231125-handshake-notification-message).

##### 2:3.112.9.1 Trigger Events

When the Resource Notification Recipient receives the [Handshake Notification Message](#231125-handshake-notification-message) from the Resource Notification Broker, it send this message to acknowledging the reception of the notification.

##### 2:3.112.9.2 Message Semantics

When the Resource Notification Recipient has processed the request, it shall return an HTTP response with an overall status code and produce a response that shall conform to requirements defined in the HL7® FHIR® standard for “[create](https://hl7.org/fhir/R4B/http.html#create)” interaction. 
...
If the Resource Notification Broker receives a negative response (based on HTTP status or the OperationOutcome receives), it shall deactivate the Subscription that triggered the handshake process, setting the `Subscription.status=error`.

<!-- response 2-->

#### 2:3.112.10 Heartbeat Notification Response

The Resource Notification Recipient sends a Heartbeat Notification Response to respond to a [Heartbeat Notification Message](#231126-heartbeat-notification-message).

##### 2:3.112.10.1 Trigger Events

When the Resource Notification Recipient receives the [Heartbeat Notification Message](#231126-heartbeat-notification-message) from the Resource Notification Broker, it send this message to acknowledge the reception of the notification.
...
If the Resource Notification Broker receives a negative response (based on HTTP status or the OperationOutcome receives), it should deactivate the Subscription that triggered the heartbeat process, setting the `Subscription.status=error`.

The recommended way for the Resource Notification Broker to manage an error in the Heartbeat Notification Response is to continue to send the Heartbeat Notification Event for a reasonable number of times, and evaluating if errors continue before deactivating the Subscription, setting the `Subscription.status=error`. This should be done based on the capability of the connection infrastructure.

<!-- response 3-->

#### 2:3.112.11 Event Notification Response

The Resource Notification Recipient sends an Event Notification Response to respond to an [Event Notification Message](#231127-event-notification-message).

##### 2:3.112.11.1 Trigger Events

When the Resource Notification Recipient receives the [Event Notification Message](#231127-event-notification-message) from the Resource Notification Broker, it send this message to acknowledge the reception of the notification.
...
If the Resource Notification Broker receives a negative response (based on HTTP status or the OperationOutcome receives), it shall deactivate the Subscription that triggered the notification process, setting the `Subscription.status=error`.

The recommended way for the Resource Notification Broker to manage an error in the Event Notification Response is to continue to send the Event Notification Event for a reasonable number of times, and evaluating if errors continue before deactivating the Subscription, setting the `Subscription.status=error`. This should be done based on the capability of the connection infrastructure.

<!-- response 4-->

#### 2:3.112.12 Subscription Deactivation Notification Response

The Resource Notification Recipient sends an Event Notification Response to respond to a [Subscription Deactivation Message](#231128-subscription-deactivation-notification-message).

##### 2:3.112.12.1 Trigger Events

When the Resource Notification Recipient receives the [Subscription Deactivation Message](#231128-subscription-deactivation-notification-message) from the Resource Notification Broker, it send this message to acknowledge the reception of the notification.
...
### 2:3.112.13 CapabilityStatement Resource
...
### 2:3.112.14 Security Considerations
...
#### 2:3.112.14.1 Security Audit Considerations
