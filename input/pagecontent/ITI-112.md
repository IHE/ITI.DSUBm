...
If the Resource Notification Broker receives a negative response (based on HTTP status or the OperationOutcome receives), it shall deactivate the Subscription  setting the `Subscription.status=error`.
...
If the Resource Notification Broker receives a negative response (based on HTTP status or the OperationOutcome receives), it should deactivate the Subscription  setting the `Subscription.status=error`.
