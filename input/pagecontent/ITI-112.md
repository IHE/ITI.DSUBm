...
The Resource Notification Recipient processes the message according to application-defined rules and produces an [Handshake Notification Response](#231129-handshake-notification-response).
...
- [Heartbeat Notification](StructureDefinition-IHE.DSUBm.HeartbeatNotification.html)
    - the `Bundle.type` element valued `history`;
    - in the first the [Heartbeat Subscription Status](StructureDefinition-IHE.DSUBm.SubscriptionStatus-Heartbeat.html) resource and the `request` element shall be filled out to match a request to the $status operation;
    - no other entry shall be present.    
    

##### 2:3.112.6.3 Expected Actions

The Resource Notification Recipient processes the message according to application-defined rules and produces an [Hearthbeat Notification Response](#2311210-hearthbeat-notification-response).

If the Resource Notification Broker has not been able to send the [Hearthbeat Notification Message](#231126-heartbeat-notification-message) because of connection problem, it should deactivate the Subscription, setting the `Subscription.status=error`. 
...
The recommended way for the Resource Notification Broker to manage these two type of error is to continue to send the Heartbeat Notification Event for a reasonable number of time, and evaluating if these type of errors continue before deactivate the Subscription, setting the `Subscription.status=error`. This should be done basing on the capability of the connection infrastruture.
...
The recommended way for the Resource Notification Broker to manage these two type of error is to continue to send the Event Notification Event, if events occur, for a reasonable number of time, and evaluating if these type of errors continue before deactivate the Subscription, setting the `Subscription.status=error`. This should be done basing on the capability of the connection infrastruture.
...
The Resource Notification Recipient sends an Handshake Notification Response to respond to an [Handshake Notification Message](#231125-handshake-notification-message).
...
If the Resource Notification Broker receives a negative response (based on HTTP status or the OperationOutcome receives), it shall deactivate the Subscription that trigger the handshake process, setting the `Subscription.status=error`.

<!-- response 2-->

#### 2:3.112.10 Hearthbeat Notification Response

The Resource Notification Recipient sends an Hearthbeat Notification Response to respond to an [Hearthbeat Notification Message](#231126-heartbeat-notification-message).

##### 2:3.112.10.1 Trigger Events

When the Resource Notification Recipient receives the [Hearthbeat Notification Message](#231126-heartbeat-notification-message) from the Resource Notification Broker, it send this message to acknowledging the reception of the notification.
...
If the Resource Notification Broker receives a negative response (based on HTTP status or the OperationOutcome receives), it should deactivate the Subscription that trigger the handshake process, setting the `Subscription.status=error`.

The recommended way for the Resource Notification Broker to manage an error in the Hearthbeat Notification Response is to continue to send the Heartbeat Notification Event for a reasonable number of time, and evaluating if errors continue before deactivate the Subscription, setting the `Subscription.status=error`. This should be done basing on the capability of the connection infrastruture.
...
When the Resource Notification Recipient receives the [Event Notification Message](#231127-event-notification-message) from the Resource Notification Broker, it send this message to acknowledging the reception of the notification.
...
The recommended way for the Resource Notification Broker to manage an error in the Event Notification Response is to continue to send the Event Notification Event for a reasonable number of time, and evaluating if errors continue before deactivate the Subscription, setting the `Subscription.status=error`. This should be done basing on the capability of the connection infrastruture.

<!-- response 4-->

#### 2:3.112.12 Subscription Deactivation Notification Response

The Resource Notification Recipient sends an Event Notification Response to respond to an [Subscription Deactivation Message](#231128-subscription-deactivation-notification-message).

##### 2:3.112.12.1 Trigger Events

When the Resource Notification Recipient receives the [Subscription Deactivation Message](#231128-subscription-deactivation-notification-message) from the Resource Notification Broker, it send this message to acknowledging the reception of the notification.
