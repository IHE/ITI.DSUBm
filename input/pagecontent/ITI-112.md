...
- [Subscription Deactivation Notification]()<!--link-->
    - the `Bundle.type` element valued `history`;
    - in the first entry the [Deactivation Subscription Status]()<!--link--> resource and the `request` element shall be filled out to match a request to the $status operation;
    - no other entry shall be present.
