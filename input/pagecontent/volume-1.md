...
1. When the patient is hospitalized The Hospital EHR sends the information to the Nurse Tablet. The Nurse Tablet perform an automated subscription to the Central Infrustructure in order to know when there is a new document available. (Mobile Subsciption ITI-Y1).
2. After requiring the blood tests to the Laboratory System the Hospital EHR perform an automated subscription to the Central Infrustructure in order to know when the results are available. (Mobile Subsciption ITI-Y1).
...
6. The doctor after receiving the notification on the Hospital EHR can retrieve and consult the analysis results on the Hospital EHR in order to deicde  which medicine is suitable for the patient (Retrieve Document [ITI-68]).
7. The doctor from the Hospital EHR makes a prescription and a document is submitted to the Central Infrastructure. (Provide  Document Bundle [ITI-65]).
8. The Central Infrastructure, having stored the metadata of the prescription, generates a message to inform the broker about the publication event. ([ITI-Y2] Mobile Publishing)
9. Since the the publication event of theprescription meets the subscription criteria performed by Nusrse tablet the Central Infrastructure will send a notification to the nurse Tablet. ([ITI-Y3] Mobile Notify)
10. The Nurse upon seeing the notification can retrieve the document in order to know which drug to prepare for the administration. (Retrieve Document [ITI-68]).
...
12. When the nurse is preparing the room for a new patient, the nurse procede to search for the previous subscription.  (Mobile Subscription Search [ITI-Y4]).  
13. The nurse can confirm the unsubrscribe from the Nurse Tablet. (Mobile Subscription Search [ITI-Y1]).
...
Mr. Brown went to see his doctor. During the examination the doctor decided that he will choose which prescription to make after seeing the blood test results. Meanwhile Mr. Brown is sent home because he has already submitted a subscription in order to recieve a notification on his mobile app when the presciption will be ready.
...
1. After the first login the mobile app performs an automatic subscription to the Central Infrastructure in order to be informed when a prescription is ready.
2. When the doctor make the prescption the e-prescription software produce a document on the Repository and sends the metadata to the Central Infrastructure ([ITI-42] Register Document Set-b).
