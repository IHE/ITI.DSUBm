...
The Resource Notification Subscriber Request Message is a parametrized `HTTP GET` that allows to search for a list of `Basic` resources managed by the Resource Notification Broker, based on a set of search parameters.

##### 2:3.114.4.1.1 Trigger Events

A Resource Notification Subscriber sends this message to the Resource Notification Broker when it needs to discover a `Basic` resource. This normally happens before the Subscriber creates a `Subscription`.
...
|[resource](SearchParameter-SubscriptionTopic-resource.html)	|uri	|	Allowed Data type or Resource (reference to definition) for this definition, searches resourceTrigger, eventTrigger, and notificationShape for matches.|
...
- [Resource Notification Subscriber SubscriptionTopic Query](StructureDefinition-IHE.DSUBm.SubscritionTopicSearch.Audit.Subscriber.html), when a search interaction is performed;
- [Resource Notification Subscriber SubscriptionTopic Read](StructureDefinition-IHE.DSUBm.SubscritionTopicRead.Audit.Subscriber.html), when a read interaction is performed.
...
- [Resource Notification Broker SubscriptionTopic Query](StructureDefinition-IHE.DSUBm.SubscritionTopicSearch.Audit.Broker.html), when a search interaction is performed;
- [Resource Notification Broker SubscriptionTopic Read](StructureDefinition-IHE.DSUBm.SubscritionTopicRead.Audit.Broker.html), when a read interaction is performed.
