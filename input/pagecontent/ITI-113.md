This section corresponds to Resource Subscription Search [ITI-113] transaction of the IHE Technical Framework. The Resource Subscription Search [ITI-113] transaction is used by the Resource Notification Subscriber and Resource Notification Broker actors. The Resource Subscription Search [ITI-113] transaction is used to query Subscription and get back the results. 
...
<figcaption><b>Figure 2:3.113.3-1: ITI-113 interactions</b></figcaption>
</figure>
<br clear="all">

### 2:3.113.5 Subscription Search Request Message
The Resource Notification Subscriber Request Message is a parametrized `HTTP GET` that allows to search from the Resource Notification Broker informations about Subscriptions.

#### 2:3.113.5.1 Trigger Events
...
The search target URL follows the FHIR HTTP specification <https://hl7.org/fhir/R4B/http.html>, addressing the Subscription Resources that the Resource Notification Subscriber is interested to discover. 
...
##### 2:3.113.5.2.1 Subscription Search Request Message examples
...
After receved a Subscription Search request, the Resource Notification Broker shall evaluate to return in response a Bundle with the Subscription Resources that matches the query parameters in the request message.

### 2:3.113.6 Subscription Status Search Request Message

The Resource Notification Subscriber Request Message is a parametrized `HTTP GET` that allows to search or retrieve, from the Resource Notification Broker, informations about the status of the Subscriptions.

#### 2:3.113.6.1 Trigger Events
...
#### 2:3.113.6.2 Message Semantics

The Resource Notification Subscriber sends an `HTTP GET` request to the Resource Notification Broker. 

The Resource Notification Subscriber Request Message could be a FHIR operation request as defined in FHIR (http://hl7.org/fhir/operations.html) with the $status operation definition and the input parameters in Table 3.113.6.2-1, in order to be aware of the Subscriptions status.
...
**Table 2:3.113.6.2-1: Subscription Status Search Request Message Parameters**
...
#### 2:3.113.6.3 Expected Actions

The Resource Notification Broker who received the message shall process the request and respond with a Resource Subscription Search Response Message. 

After receved a Subscription Status Search Request Message, the Resource Notification Broker shall evaluate to return in response a Bundle with the SubscriptionStatus Resources related to the Subscription that matches the query parameters in the request message.


### 2:3.113.7 Subscription Events Search Request Message
The Resource Notification Subscriber Request Message is a parametrized `HTTP GET` that allows to retrieve, from the Resource Notification Broker, the events about a specific Subscription that could have been occurs.

#### 2:3.113.7.1 Trigger Events
...
#### 2:3.113.7.2 Message Semantics

The Resource Notification Subscriber sends an `HTTP GET` request to the Resource Notification Broker. 

The Resource Notification Subscriber Request Message could be a FHIR operation request as defined in FHIR (http://hl7.org/fhir/operations.html) with the $events operation definition and the input parameters in Table 3.113.7.2-1, in order to be aware of previous events related to a Subscription.

The URL for this operation is:

>GET  \[base\]/Subscription/\[id\]/$events

**Table 2:3.113.7.2-1: Subscription Events Search Request Message Parameters**
...
#### 2:3.113.7.3 Expected Actions

The Resource Notification Broker who received the message shall process the request and respond with a Resource Subscription Search Response Message. 

After receved a Subscription Events Search Request Message, the Resource Notification Broker shall evaluate to return in response a Bundle with the SubscriptionStatus Resource and the previous events related to the Subscription that match the query parameters in the request message. The Resource Notification Broker may use implementation-specific criteria to restrict availability of events (e.g., most recent 10 events, events within the past 30 days, only the events from the last $events operation performd by a Resource Notification Subscriber, etc.).


### 2:3.113.8 Subscription Search Response Message
...
If the Resource Subscription Search message is processed successfully, whether or not any Subscription Resources are found based on the request parameter, the response shall be an `HTTP 200` status code. The Resource Subscription Search Response message shall be a Bundle Resource that shall have:
 ...
 More information about search response can be found at [http://hl7.org/fhir/R4B/http.html#search](http://hl7.org/fhir/R4B/http.html#search).


#### 2:3.113.8.3 Expected Actions
The Resource Notification Subscriber shall process the results according to application-defined rules.


### 2:3.113.9 Subscription Status Search Response Message
...
#### 2:3.113.9.3 Expected Actions
The Resource Notification Subscriber shall process the results according to application-defined rules.



### 2:3.113.10 Subscription Events Search Response Message
...
If the $events operation request is processed successfully the response shall be an `HTTP 200` status code. The Resource Subscription Search Response message shall be a Bundle Resource that shall have:
 ...
 - in the first the first entry the [SubscriptionStatus]() for the $events operation;
...
### 2:3.113.10.4 CapabilityStatement Resource
...
### 2:3.113.10.5 Security Considerations

See [DSUBm Security Considerations](volume-1.html#1545-security-considerations). 

The implementers should be aware that a specific Resource Notification Subscriber may acquire sensitive information if searching for Subscription created by other Subscribers. 
...
#### 2:3.113.10.5.1 Security Audit Considerations
