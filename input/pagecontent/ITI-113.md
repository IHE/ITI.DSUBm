...
The Resource Notification Subscriber Request Message could be a FHIR operation request as defined in FHIR (http://hl7.org/fhir/operations.html) with the $status operation definition and the input parameters in Table 3.113.5.2.2-1:
...
The Resource Notification Subscriber Request Message could be a FHIR operation request as defined in FHIR (http://hl7.org/fhir/operations.html) with the $event operation definition and the input parameters in Table 3.113.5.2.3-1:
...
If receved a $status operation request, the Resource Notification Broker shall evaluate to return in response a Bundle with the SubscriptionStatus Resurces that matches the query parameters in the request message.
...
If the Resource Subscription Search message is processed successfully, whether or not any Subscription Resources are found based on the request parameter, the response shall be an `HTTP 200` status code. The Resource Subscription Search Response message shall be a Bundle Resource
 containing zero or more Subscription Resources. If the Document Responder is sending warnings, the Bundle Resource could also contain an OperationOutcome Resource that contains those warnings.

If the $status operation request is processed successfully the response shall be an `HTTP 200` status code. The Resource Subscription Search Response message shall be a Bundle Resource containing zero or more SubscriptionStatus Resources. If the Document Responder is sending warnings, the Bundle Resource could also contain an OperationOutcome Resource that contains those warnings.

If the $event operation request is processed successfully the response shall be an `HTTP 200` status code. The Resource Subscription Search Response message shall be a Bundle Resource containing as the first entry the SubscriptionStatus Resources related to the Subscription and. If the Document Responder is sending warnings, the Bundle Resource could also contain an OperationOutcome Resource that contains those warnings.
