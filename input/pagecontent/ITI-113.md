...
The query parameters allowed, reported in Table 3.113.5.2.1-1,  are the parameters indicated in the [Search parameters](https://hl7.org/fhir/R4/subscription.html#search) section and also the [search parameter](https://build.fhir.org/ig/HL7/fhir-subscription-backport-ig/artifacts.html#behavior-search-parameters) included in the [Subscription R5 Backport Implementation Guide](http://hl7.org/fhir/uv/subscriptions-backport/STU1.1/).  
...
If receved a Subscription Search request, the Resource Notification Broker shall evaluate to return in response a Bundle with the Subscription Resources that matches the query parameters in the request message.

If receved a $status operation request, the Resource Notification Broker shall evaluate to return in response a Bundle with the SubscriptionStatus Resources related to the Subscription that matches the query parameters in the request message.

If receved a $event operation request, the Resource Notification Broker shall evaluate to return in response a Bundle with the SubscriptionStatus Resource and the previous events related to the Subscription that match the query parameters in the request message. The Resource Notification Broker may use implementation-specific criteria to restrict availability of events (e.g., most recent 10 events, events within the past 30 days, only the events from the last $event operation performd by a Resource Notification Subscriber, etc.).
...
If the $event operation request is processed successfully the response shall be an `HTTP 200` status code. The Resource Subscription Search Response message shall be a Bundle Resource that shall have:
 ...
 - in the first the first entry the [SubscriptionStatus Parameter]() for the $event operation;
